...
```
<!-- B站收藏-->
			<div class="left-div left-time" id="bilibiliContainer">
				<ul id="line4">
					<div class="container">
						<class="title">《我的B站收藏夹视频》
							<hr class="dashed">
					</div>
					<!-- 视频播放器将在这里动态插入 -->
					<div id="bilibiliVideoContainer">
					</div>
			</div>
			<script src="./js/bilibili-video.js"></script>
			<script>
				let currentBilibiliIndex = 0; 

				function loadBilibiliVideo(index) {
					...
					const bilibiliVideoContainer = document.getElementById('bilibiliVideoContainer');
					bilibiliVideoContainer.innerHTML = '';
					bilibiliVideoContainer.appendChild(iframe);
				}

				function loadRandomBilibiliVideo() {
					currentBilibiliIndex = Math.floor(Math.random() * videoList.length);
					loadBilibiliVideo(currentBilibiliIndex);
				}

				// 页面加载时随机播放视频
				window.onload = loadRandomBilibiliVideo;
			</script>
<!-- B站收藏结束-->
```

该方法为随机播放你存在js文件中的视频链接
